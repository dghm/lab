...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_bill') {
        ...
        if (!$isFixed && (!$ps || !$pe)) {
            ...
            $attachment = save_bill_attachment($_FILES['attachment'] ?? []);
            if (($_FILES['attachment']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE && $attachment === null) {
                $flash = '附件上傳失敗：僅支援 PDF / JPG / PNG。帳單其餘內容未儲存，請重新送出。';
            } else {
                $stmt = db()->prepare(
                  'INSERT INTO bills (settlement_id, category, period_start, period_end, total_amount, split_base, is_fixed, note, attachment_path)
                   VALUES (?,?,?,?,?,?,?,?,?)');
                $stmt->execute([
                    (int)$_POST['settlement_id'], trim($_POST['category']),
                    $ps, $pe, $total, $base, $isFixed, trim($_POST['note'] ?? ''), $attachment
                ]);
                $flash = '已新增帳單，分攤已自動計算。';
            }
        }
    ...
    if ($action === 'delete_bill') {
        ...
        if ($old) delete_bill_attachment($old['attachment_path']);
        $flash = '已刪除該筆帳單。';
    }
...
<body>
  ...
  <main class="wrap">
    ...
    <?php if ($active): $bills = bills_of($activeId); $due = tenant_due($activeId); ?>
            ...
            <tr>
              ...
              <td class="r">
                <?php if (!empty($b['attachment_path'])): ?>
                  <a class="btn-copy" href="bill-file.php?id=<?= $b['id'] ?>" target="_blank" rel="noopener" title="查看附件">📄</a>
                <?php endif; ?>
                <form method="post" onsubmit="return confirm('確定刪除這筆帳單？');" style="margin:0;display:inline">
      ...
      <section class="card">
        <h3>新增帳單</h3>
        <form method="post" class="form grid2" id="bill-form" enctype="multipart/form-data">
          <input type="hidden" name="csrf" value="<?= h(csrf_token()) ?>">
          ...
          <label class="full">帳單附件（選填，PDF 或 圖片，房客可在卡片內查看）

  ...
  function calc() {
    ...
    if (fixed) {
      ...
      preview.innerHTML = '固定費：分攤基礎 NT$' + base.toLocaleString() +
        ' ÷ 3 = <b>每人 NT$' + perF + '</b>　·　房東負擔 NT$' + (round(total) - 2 * perF);
      return;
    }
    ...
    preview.innerHTML =
      '總 ' + totalDays + ' 天（前段 ' + daysBefore + ' 天房東自付、後段 ' + daysAfter + ' 天均分）<br>' +
      '後段金額 = ' + base + ' × (' + daysAfter + ' ÷ ' + totalDays + ') = NT$' + round(back).toLocaleString() + '<br>' +
      '每人 = ' + round(back).toLocaleString() + ' ÷ 3 = <b>NT$' + per + '</b>　·　房東負擔 NT$' + landlord.toLocaleString();
  }
